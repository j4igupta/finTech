...
function PlayerAvatar({
  ...
  return (
    <div
      className={cn(
        "relative h-10 w-10 overflow-hidden rounded-full bg-secondary",
        glowClass
      )}
    >
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-border bg-gradient-to-r from-card via-[#1E90FF]/5 to-[#FF69B4]/5 p-5">
        ...
        <button className="text-xs font-medium text-[#1E90FF] hover:underline">
          View All
        </button>
      </div>
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-border hover:bg-transparent">
            <TableHead className="w-16 text-center text-xs text-muted-foreground">
              Rank
            </TableHead>
        ...
        <TableBody>
          {players.map((player, index) => (
            <TableRow
              ...
              className={cn(
                ...
                player.isCurrentUser &&
                  "bg-[#1E90FF]/10 hover:bg-[#1E90FF]/15 border-l-2 border-l-[#1E90FF]",
                player.rank <= 3 &&
                  !player.isCurrentUser &&
            ...
            >
              ...
              {/* Win streak */}
              <TableCell className="text-center">
                <WinStreakBadge streak={player.winStreak} />
              </TableCell>

              {/* Trend indicator */}
              <TableCell className="text-center">
                {player.change === "up" && (
                  <ChevronUp className="mx-auto h-5 w-5 text-[#00FFAA]" />
                )}
                {player.change === "down" && (
                  <ChevronDown className="mx-auto h-5 w-5 text-destructive" />
                )}
                {player.change === "same" && (
                  <div className="mx-auto h-1 w-4 rounded-full bg-muted-foreground/30" />
                )}
              </TableCell>
            </TableRow>
          ))}
      ...
      {/* Footer with current user position if not in top list */}
