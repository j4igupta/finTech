      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((message) => (
            <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm ${message.role === "user" ? "bg-gradient-to-r from-[#1E90FF] to-[#1E90FF]/80 text-white" : "bg-secondary text-secondary-foreground"}`}>
                {message.content}
              </div>
            </div>
          ))}
        ...
        {messages.length === 1 && (
          <div className="flex gap-2 overflow-x-auto px-4 pb-2 scrollbar-hide">
            {suggestions.map((suggestion, i) => (
              <button key={i} onClick={() => setInput(suggestion)} className="flex-shrink-0 rounded-full border border-border bg-secondary/50 px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:border-[#1E90FF]/50 hover:text-foreground">
                {suggestion}
              </button>
            ))}
          </div>
        ...
        <div className="border-t border-border p-3">
          <div className="flex items-center gap-2">
            ...
            <Button size="icon" onClick={handleSend} disabled={!input.trim()} className="h-10 w-10 rounded-xl bg-gradient-to-r from-[#1E90FF] to-[#FF69B4] text-white hover:opacity-90 disabled:opacity-50">
