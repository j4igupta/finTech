...
export function QuestList({ quests }: DailyQuestsProps) {
              ...
              <div className="flex items-start gap-3">
                <div
                  className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg ${
                    quest.completed ? "bg-[#00FFAA]/20 text-[#00FFAA]" : "bg-secondary text-muted-foreground"
                  }`}
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <p className={`text-sm font-medium ${quest.completed ? "text-[#00FFAA]" : ""}`}>{quest.title}</p>
                      ...
                      <div className="h-1.5 w-full overflow-hidden rounded-full bg-secondary">
                        <div
                          className={`h-full rounded-full transition-all ${
                            quest.completed ? "bg-[#00FFAA]" : "bg-gradient-to-r from-[#1E90FF] to-[#FF69B4]"
                          }`}
                          style={{ width: `${(quest.progress / quest.total) * 100}%` }}
                        />
                      </div>
                    </div>
                    <span className="text-xs font-medium text-[#FF69B4]">+{quest.xpReward} XP</span>
                  </div>
                </div>
              </div>
            </div>
